    ...
    try:
        ...
        for j in jugadas:
            if j['tipo']!='tripleta' and not puede_vender(j['hora']):
                return jsonify({'error':f"Sorteo {j['hora']} ya cerró (5 min antes)"}),400
        ...
        with get_db() as db:
            ...
            # Verificar topes por número
            for j in jugadas:
                if j['tipo'] == 'animal':
                    tope_row = db.execute(
                        "SELECT monto_tope FROM topes WHERE hora=? AND numero=?",
                        (j['hora'], j['seleccion'])
                    ).fetchone()
                    if tope_row:
                        # Hay tope configurado - verificar acumulado
                        ...
                        """, (j['hora'], j['seleccion'], hoy+'%')).fetchone()['tot']
                        if ya_apostado + j['monto'] > tope_row['monto_tope']:
                            nombre = ANIMALES.get(j['seleccion'], j['seleccion'])
                            return jsonify({'error':f'Tope alcanzado para {j["seleccion"]}-{nombre} en {j["hora"]}. Disponible: S/{tope_row["monto_tope"]-ya_apostado:.2f}'}),400
                    # Si no hay tope = jugada libre, se permite cualquier monto
        ...
        # Generar texto para WhatsApp — solo hora Perú + nombre Perú
        ...
        # Animales normales y especiales — solo Perú
        ...
        # Tripletas — 11 sorteos día en curso
        ahora_dt = ahora_peru()
        ...
        if trips_t:
            ...
            for t in trips_t:
                ...
                hora_ini = HORARIOS_PERU[0]   # 08:00 AM
                hora_fin = HORARIOS_PERU[-1]  # 06:00 PM
        ...
        lineas += ["------------------------",
                   f"*TOTAL: S/{fmt(total)}*",
                   "",
                   "ZOOLO CASINO: Valido 11 sorteos fijos del dia (8AM-6PM)" if trips_t else "",
                   "Buena Suerte! 🍀",
    ...
    try:
        ...
        with get_db() as db:
            ...
            for t in rows:
                ...
                trips_det = []
                res_validos_trip = resultados_validos_para_tripleta(res_dia, dt)
...
def consultar_ticket_detalle():
    try:
        ...
        res_validos_trip = resultados_validos_para_tripleta(res_dia, fecha_ticket_dt)
        for tr in tripletas_raw:
            nums={tr['animal1'],tr['animal2'],tr['animal3']}
            salidos=list(dict.fromkeys([a for a in res_validos_trip.values() if a in nums]))
            gano_t=len(salidos)==3; pt=tr['monto']*PAGO_TRIPLETA if gano_t else 0
...
def guardar_resultado():
    try:
        ...
        with get_db() as db:
            # Validar que el animal no haya salido ya hoy en OTRO sorteo
            ya_salio = db.execute(
                "SELECT hora FROM resultados WHERE fecha=? AND animal=? AND hora!=?",
                (fecha, animal, hora)
            ).fetchone()
            if ya_salio:
                nombre_animal = ANIMALES[animal]
                return jsonify({
                    'error': f'⚠️ El animal {animal}-{nombre_animal} ya salió hoy en el sorteo de {ya_salio["hora"]}. '
        ...
        log_audit('RESULTADO', f"Fecha:{fecha} Hora:{hora} Animal:{animal} ({ANIMALES[animal]})")
        return jsonify({
            'status':'ok',
...
def get_topes():
    try:
        hora = request.args.get('hora', HORARIOS_PERU[0])
        hoy = ahora_peru().strftime("%d/%m/%Y")
        with get_db() as db:
            topes_rows = db.execute(
                "SELECT numero, monto_tope FROM topes WHERE hora=? ORDER BY CAST(numero AS INTEGER) ASC",
                (hora,)
            ).fetchall()
            # Calcular acumulado actual por número para esa hora hoy
            jugadas_rows = db.execute("""
                SELECT jg.seleccion, COALESCE(SUM(jg.monto),0) as apostado
                FROM jugadas jg
                JOIN tickets tk ON jg.ticket_id=tk.id
                WHERE jg.hora=? AND jg.tipo='animal' AND tk.anulado=0 AND tk.fecha LIKE ?
        ...
        # Build list: only numbers that have a tope OR have been played
...
def guardar_tope():
    try:
        ...
        with get_db() as db:
            if monto <= 0:
                db.execute("DELETE FROM topes WHERE hora=? AND numero=?", (hora, numero))
                log_audit('TOPE_LIBRE', f"Hora:{hora} Num:{numero} puesto en libre")
            else:
                db.execute("""
                    INSERT INTO topes (hora, numero, monto_tope) VALUES (?,?,?)
                    ON CONFLICT(hora,numero) DO UPDATE SET monto_tope=excluded.monto_tope
                """, (hora, numero, monto))
                log_audit('TOPE_SET', f"Hora:{hora} Num:{numero}-{ANIMALES[numero]} Tope:S/{monto}")
            db.commit()
        return jsonify({'status':'ok'})
    except Exception as e:
        return jsonify({'error':str(e)}),500
...
def limpiar_topes():
    try:
        ...
        if hora not in HORARIOS_PERU:
            return jsonify({'error':'Hora inválida'}),400
        with get_db() as db:
            db.execute("DELETE FROM topes WHERE hora=?", (hora,))
            db.commit()
...
def riesgo():
    try:
        ...
        with get_db() as db:
            ...
            # Aggregate bets by number for the selected sorteo
            jugadas_rows = db.execute("""
                SELECT jg.seleccion, COALESCE(SUM(jg.monto),0) as apostado
                FROM jugadas jg
                JOIN tickets tk ON jg.ticket_id=tk.id
            ...
            topes_rows = db.execute("SELECT numero, monto_tope FROM topes WHERE hora=?", (sorteo,)).fetchall()
            topes_map = {r['numero']: r['monto_tope'] for r in topes_rows}
...
.left-panel{display:flex;flex-direction:column;width:55%;border-right:2px solid var(--border);overflow:hidden;background:#ffffff}
...
/* PANEL DERECHO */
.right-panel{width:45%;display:flex;flex-direction:column;overflow-y:auto;background:#f8fafc;padding:6px}
...
.ticket-list{flex:1;overflow-y:auto;min-height:40px;background:#f8fafc;border:1px solid #e2e8f0;border-radius:4px;padding:4px;margin-bottom:4px}
...
<div class="layout">
  ...
  <div class="right-panel">
    ...
    <!-- HORARIOS -->
    <div class="rsec">
      <div class="rlabel">⏰ Horarios (Cierra 5 min antes)</div>
...
<div class="mc">
  <div class="mh">
    <h3>🎯 JUGAR TRIPLETA x60 (8AM-6PM — 11 SORTEOS)</h3>
    <button class="btn-close" onclick="closeMod('mod-tripleta')">✕</button>
  </div>
  <div class="mbody">
    <div style="color:var(--text2);font-size:.75rem;margin-bottom:10px;text-align:center">Válida solo para hoy — Sorteos 8AM a 6PM (11 sorteos)</div>
...
function agregarTripletaModal(){
  ...
  carrito.push({tipo:'tripleta',hora:'TODO DÍA (8AM-6PM)',seleccion:sel,monto,desc:'🎯 '+desc});
  
  renderCarrito();
  closeMod('mod-tripleta');
  toast('Tripleta agregada (válida 8AM-6PM)','ok');
}
...
function agregarItemRepetir(idx, tipo, seleccion){
  ...
  if(tipo==='tripleta'){
    ...
    carrito.push({tipo:'tripleta',hora:'TODO DÍA (8AM-6PM)',seleccion:seleccion,monto,desc:'🎯 '+desc});
...
function renderCarrito(){
  ...
  carrito.forEach((it,i)=>{
    total+=it.monto;
    let horaLabel=it.hora==='TODO DÍA (8AM-6PM)'?'x60':it.hora.replace(':00','').replace(' ','');
    html+=`<div class="ti">
...
<div id="tc-tripletas" class="tc">
  <div class="fbox">
    <h3>🔮 TRIPLETAS HOY (8AM-6PM — 11 SORTEOS)</h3>
...
function cargarRiesgo(){
  ...
  fetch('/admin/riesgo?hora='+encodeURIComponent(hora)).then(r=>r.json()).then(d=>{
...
function cargarTopes(){
  ...
  fetch('/admin/topes?hora='+encodeURIComponent(hora)).then(r=>r.json()).then(d=>{
...
function guardarTope(){
  ...
  fetch('/admin/topes/guardar',{method:'POST',headers:{'Content-Type':'application/json'},
    body:JSON.stringify({hora,numero:num,monto})})
  .then(r=>r.json()).then(d=>{
    if(d.status==='ok'){
...
function limpiarTopesHora(){
  let hora=document.getElementById('tope-hora-sel').value;
  if(!confirm(`¿Eliminar todos los topes de ${hora}?`))return;
  fetch('/admin/topes/limpiar',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({hora})})
  .then(r=>r.json()).then(d=>{
