...
# ZOOLO CASINO: 11 sorteos fijos de 8AM a 6PM (solo Perú, no tiene VZ offset)
HORARIOS_ZOOLO = [
    "08:00 AM","09:00 AM","10:00 AM","11:00 AM","12:00 PM",
    "01:00 PM","02:00 PM","03:00 PM","04:00 PM","05:00 PM","06:00 PM"
]

# Otras loterías sin offset: 12 sorteos 7AM-6PM
# ZOOLO CASINO: 11 sorteos fijos de 8AM a 6PM
HORARIOS_ZOOLO = [
    "08:00 AM","09:00 AM","10:00 AM","11:00 AM","12:00 PM",
    "01:00 PM","02:00 PM","03:00 PM","04:00 PM","05:00 PM","06:00 PM"
]
...
# Loterías con offset_30min: 12 sorteos 7:30AM-6:30PM
...
def procesar_venta():
    try:
        ...
        def fmt_h(h):
            """Convierte '01:00 PM' -> '1PM', '12:30 PM' -> '12:30PM' """
            h2 = h.strip()
            # Quitar :00 solo si los minutos son 00
            import re
        ...
        def nom_loteria_ticket(lot_id, lot):
            """Nombre corto del ticket según lotería"""
            nombres = {
                'zoolo':    'ZOOLOCAS',
                'activo':   'L-ACTIVO',
                'granjita': 'GRANJITA',
        ...
        for lot_id, jugs in por_loteria.items():
            ...
            horas_peru, horas_venezuela = get_horarios(lot_id)
            por_hora = defaultdict(list)
            for j in jugs: por_hora[j['hora']].append(j)
            es_zoolo = (lot_id == 'zoolo')

            for hp in horas_peru:
                if hp not in por_hora: continue
                hv_idx = horas_peru.index(hp)
                hv = horas_venezuela[hv_idx] if hv_idx < len(horas_venezuela) else ''
                hpc = fmt_h(hp)
                hvc = fmt_h(hv) if hv else ''
                nom = nom_loteria_ticket(lot_id, lot)

                # ZOOLO solo hora Perú; el resto muestra PERU/VZLA
                if es_zoolo:
                    lineas.append(f"{nom} {hpc}....PERU")
                else:
                    lineas.append(f"{nom} {hpc}.PERU--{hvc}.VZLA")

                # Agrupar jugadas del mismo número sumando montos
        ...
        if trips:
            ...
            for t in trips:
                ...
                if hora_inicio is None:
                    hora_inicio = horas_peru_t[0] if horas_peru_t else "07:00 AM"

                idx_inicio = horas_peru_t.index(hora_inicio) if hora_inicio in horas_peru_t else 0

                if es_zoolo_trip:
                    # ZOOLO: tripleta válida para los 11 sorteos fijos del día (08AM - 6PM)
                    # DESDE y HASTA son siempre fijos: primer y último sorteo de ZOOLO
                    hora_inicio_zoolo = HORARIOS_ZOOLO[0]   # "08:00 AM"
                    hora_fin_zoolo    = HORARIOS_ZOOLO[-1]  # "06:00 PM"
                    fecha_hoy_fmt     = ahora_dt.strftime("%d/%m/%Y")
                    lineas.append(f"-------{nom_trip}----------")
                    lineas.append(f"DESDE {fecha_hoy_fmt} Sorteo {fmt_h(hora_inicio_zoolo)} PERU")
                    lineas.append(f"HASTA {fecha_hoy_fmt} Sorteo {fmt_h(hora_fin_zoolo)} PERU")
                    lineas.append(f"(11 sorteos fijos del dia: 8AM a 6PM)")
                else:
                    # Resto de loterias: 11 sorteos consecutivos desde hora jugada (sin pais)
                    idx_fin_raw = idx_inicio + 10
                    dias_extra  = idx_fin_raw // len(horas_peru_t)
                    idx_fin     = idx_fin_raw % len(horas_peru_t)
                    hora_fin    = horas_peru_t[idx_fin]
                    fecha_fin_dt  = ahora_dt + timedelta(days=dias_extra)
                    fecha_fin_fmt = fecha_fin_dt.strftime("%d/%m/%Y")
                    fecha_ini_fmt = ahora_dt.strftime("%d/%m/%Y")
                    lineas.append(f"-------{nom_trip} {fmt_h(hora_inicio)}---------")
                    lineas.append(f"DESDE {fecha_ini_fmt} Sorteo {fmt_h(hora_inicio)}")
                    lineas.append(f"HASTA {fecha_fin_fmt} Sorteo {fmt_h(hora_fin)}")
...
def lista_agencias():
    admin_id = session['user_id']
    with get_db() as db:
        rows = db.execute("SELECT id,usuario,nombre_agencia,comision,activa FROM agencias WHERE es_admin=0 AND es_super_admin=0 AND admin_padre_id=?", (admin_id,)).fetchall()
...
function getHorarios(lid){
  ...
  if(lot && lot.offset_30min){
    return {
      peru: ["07:30 AM","08:30 AM","09:30 AM","10:30 AM","11:30 AM","12:30 PM","01:30 PM","02:30 PM","03:30 PM","04:30 PM","05:30 PM","06:30 PM"],
      venezuela: ["08:30 AM","09:30 AM","10:30 AM","11:30 AM","12:30 PM","01:30 PM","02:30 PM","03:30 PM","04:30 PM","05:30 PM","06:30 PM","07:30 PM"]
    };
  }
  return {
    peru: ["07:00 AM","08:00 AM","09:00 AM","10:00 AM","11:00 AM","12:00 PM","01:00 PM","02:00 PM","03:00 PM","04:00 PM","05:00 PM","06:00 PM"],
    venezuela: ["08:00 AM","09:00 AM","10:00 AM","11:00 AM","12:00 PM","01:00 PM","02:00 PM","03:00 PM","04:00 PM","05:00 PM","06:00 PM","07:00 PM"]
  };
}
...
<div class="tabs">
  <div class="tab active" onclick="showTab('t-resultados',this)">📥 RESULTADOS</div>
  <div class="tab" onclick="showTab('t-limites',this)">🔒 BLOQUEOS</div>
  <div class="tab" onclick="showTab('t-dash',this)">📊 DASHBOARD</div>
  <div class="tab" onclick="showTab('t-agencias',this)">🏢 AGENCIAS</div>
  <div class="tab" onclick="showTab('t-ops',this)">🔧 OPERACIONES</div>
</div>

<div class="glmsg msg" id="glmsg"></div>

<div class="tc active" id="t-resultados">
  <div class="fbox">
    <h3>📥 CARGAR RESULTADO</h3>
    <div class="lot-tabs" id="lot-tabs-res"></div>
    <div class="frow">
      <input type="date" id="ra-fecha">
      <select id="ra-hora"></select>
    </div>
    <div style="margin-bottom:10px">
      <div style="color:var(--text2);font-size:.72rem;letter-spacing:1px;margin-bottom:6px;font-family:'Oswald',sans-serif">SELECCIONAR ANIMAL GANADOR</div>
      <div class="animals-mini" id="animals-mini-res"></div>
    </div>
    <div id="animal-sel-info" style="color:#fbbf24;font-family:'Oswald',sans-serif;font-size:.82rem;margin-bottom:8px;min-height:20px"></div>
    <div style="display:flex;gap:6px;flex-wrap:wrap">
      <button class="btn-s" onclick="guardarResultado()">💾 GUARDAR</button>
      <button class="btn-sec" onclick="cargarResultadosFecha()">🔄 VER DÍA</button>
    </div>
    <div id="res-admin-lista" style="margin-top:12px;max-height:300px;overflow-y:auto"></div>
  </div>
</div>

<div class="tc" id="t-limites">
  <div class="fbox">
...
<div class="tc" id="t-dash">
  <div class="fbox">
    <h3>📊 RESUMEN HOY</h3>
    <div class="sgrid">
      <div class="sc"><h3>VENTAS</h3><p id="d-ventas">--</p></div>
      <div class="sc"><h3>TICKETS</h3><p id="d-tickets">--</p></div>
      <div class="sc"><h3>PREMIOS PAGADOS</h3><p class="r" id="d-premios">--</p></div>
      <div class="sc"><h3>COMISIONES</h3><p id="d-comisiones">--</p></div>
      <div class="sc"><h3>BALANCE</h3><p class="g" id="d-balance">--</p></div>
    </div>
    <button class="btn-sec" onclick="cargarDash()">🔄 ACTUALIZAR</button>
  </div>
</div>

<div class="tc" id="t-agencias">
  <div class="fbox">
    <h3>➕ CREAR AGENCIA</h3>
    <div class="frow">
      <input type="text" id="ag-user" placeholder="Usuario (lowercase)">
      <input type="password" id="ag-pass" placeholder="Contraseña">
      <input type="text" id="ag-nombre" placeholder="Nombre de la agencia">
    </div>
    <button class="btn-s" onclick="crearAgencia()">CREAR AGENCIA</button>
    <div id="ag-msg" style="margin-top:6px"></div>
  </div>
  <div class="fbox">
    <h3>📋 MIS AGENCIAS</h3>
    <button class="btn-sec" onclick="cargarAgencias()" style="margin-bottom:10px">🔄 ACTUALIZAR</button>
    <div id="ag-lista"></div>
  </div>
</div>
...
<div class="tc" id="t-ops">
  <div class="fbox">
    <h3>🔍 BUSCAR / PAGAR TICKET</h3>
    <div class="frow"><input type="text" id="op-serial" placeholder="Serial del ticket"></div>
    <button class="btn-s" onclick="buscarTicketAdm()">BUSCAR</button>
    <div id="op-res" style="margin-top:10px"></div>
  </div>
  <div class="fbox">
    <h3>❌ ANULAR TICKET (Admin)</h3>
    <div class="frow"><input type="text" id="an-ser" placeholder="Serial del ticket"></div>
    <button class="btn-d" onclick="anularAdm()">ANULAR TICKET</button>
    <div id="an-res" style="margin-top:6px"></div>
  </div>
</div>

<script>
const LOTERIAS_DATA = {{ loterias | tojson }};
let lotResActual = 'zoolo';
let lotLimActual = 'zoolo';
let animalSelAdmin = null;
let animalSelLim = null;

function getHorarios(lid){
  let lot = LOTERIAS_DATA[lid];
  if(lot && lot.offset_30min){
    return ["07:30 AM","08:30 AM","09:30 AM","10:30 AM","11:30 AM","12:30 PM","01:30 PM","02:30 PM","03:30 PM","04:30 PM","05:30 PM","06:30 PM"];
  }
  return ["07:00 AM","08:00 AM","09:00 AM","10:00 AM","11:00 AM","12:00 PM","01:00 PM","02:00 PM","03:00 PM","04:00 PM","05:00 PM","06:00 PM"];
}
...
function renderLotTabs(containerId, current, onClickFn){
  let c = document.getElementById(containerId);
  if(!c) return;
  c.innerHTML = '';
  Object.entries(LOTERIAS_DATA).forEach(([k,v])=>{
    let b = document.createElement('button');
    b.className = 'lot-tab '+(k===current?'sel':'');
    b.style.borderColor = k===current?v.color:'#1a2540';
    b.style.color = k===current?v.color:'#4a6090';
    b.style.background = k===current?`color-mix(in srgb,${v.color} 10%,#050810)`:'#0a0e18';
    b.textContent = v.emoji+' '+v.nombre;
    b.onclick = ()=>onClickFn(k);
    c.appendChild(b);
  });
}
...
function selLotRes(lid){
  ...
  renderAnimalesMini(lid,'animals-mini-res',(k)=>{animalSelAdmin=k;updateSelInfo();});
  animalSelAdmin=null; document.getElementById('animal-sel-info').textContent='';
}
function selLotLim(lid){
  lotLimActual=lid;
  renderLotTabs('lot-tabs-lim',lid,selLotLim);
  updateHorarioSelect(lid,'lim-hora');
  renderAnimalesMini(lid,'animals-lim',(k)=>{animalSelLim=k;updateLimSelInfo();});
  animalSelLim=null; document.getElementById('lim-sel-info').textContent='';
  cargarLimites();
}

function updateSelInfo(){
  if(animalSelAdmin){
    let lot=LOTERIAS_DATA[lotResActual];
    document.getElementById('animal-sel-info').textContent=`✓ ${animalSelAdmin} — ${lot.animales[animalSelAdmin]}`;
  }
}
function updateLimSelInfo(){
  if(animalSelLim){
    let lot=LOTERIAS_DATA[lotLimActual];
    document.getElementById('lim-sel-info').textContent=`✓ ${animalSelLim} — ${lot.animales[animalSelLim]}`;
  }
}

function renderAnimalesMini(lid,containerId,onClickFn){
  let lot=LOTERIAS_DATA[lid]; let g=document.getElementById(containerId); g.innerHTML='';
  let animales=lot.animales;
  let orden=Object.keys(animales).sort((a,b)=>{
    let na=a==='00'?-1:parseInt(a); let nb=b==='00'?-1:parseInt(b);
    if(isNaN(na)) na=999; if(isNaN(nb)) nb=999; return na-nb;
  });
  orden.forEach(k=>{
    let d=document.createElement('div'); d.className='am-item';
    d.innerHTML=`<div class="anum">${k}</div><div class="anom">${animales[k]}</div>`;
    d.onclick=()=>{
      document.querySelectorAll('#'+containerId+' .am-item').forEach(e=>e.classList.remove('sel'));
      d.classList.add('sel'); onClickFn(k);
    };
    g.appendChild(d);
  });
}
...
function guardarResultado(){
  ...
  let fecha=document.getElementById('ra-fecha').value;
  if(!animalSelAdmin){glMsg('⚠ Seleccione un animal','err');return;}
  if(!hora){glMsg('⚠ Seleccione hora','err');return;}
  let fd=new FormData();
  fd.append('hora',hora); fd.append('animal',animalSelAdmin);
  fd.append('fecha',fecha); fd.append('loteria',lotResActual);
  fetch('/admin/guardar-resultado',{method:'POST',body:fd})
  .then(r=>r.json()).then(d=>{
    if(d.status==='ok'){glMsg('✅ '+d.mensaje,'ok');cargarResultadosFecha();}
...
function cargarResultadosFecha(){
  let fecha=document.getElementById('ra-fecha').value;
  let c=document.getElementById('res-admin-lista');
  c.innerHTML='<p style="color:var(--text2);font-size:.72rem;padding:8px">CARGANDO...</p>';
  fetch('/api/resultados-fecha',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({fecha:fecha,loteria:lotResActual})})
  .then(r=>r.json()).then(d=>{
    let lot=LOTERIAS_DATA[lotResActual];
    let horas=getHorarios(lotResActual);
    let html=`<div style="color:${lot.color};font-family:'Oswald',sans-serif;font-size:.75rem;margin-bottom:6px">${lot.emoji} ${lot.nombre} — ${d.fecha_consulta}</div>`;
    horas.forEach(h=>{
      let res=d.resultados[h];
      html+=`<div class="ri ${res?'ok':''}">
        <span style="color:#fbbf24;font-weight:700;font-family:'Oswald',sans-serif;font-size:.78rem">${h.replace(':00 ','').replace(' ','')}</span>
        ${res?`<span style="color:#4ade80;font-weight:700">${res.animal} — ${res.nombre}</span>`:'<span style="color:#1e2a40;font-size:.75rem">PENDIENTE</span>'}
      </div>`;
    });
    c.innerHTML=html;
  }).catch(()=>{c.innerHTML='<p style="color:var(--red)">Error</p>';});
...
function cargarDash(){
  fetch('/admin/reporte-agencias').then(r=>r.json()).then(d=>{
    let g=d.global;
    document.getElementById('d-ventas').textContent='S/'+g.ventas.toFixed(2);
    document.getElementById('d-premios').textContent='S/'+g.pagos.toFixed(2);
    document.getElementById('d-comisiones').textContent='S/'+g.comisiones.toFixed(2);
    document.getElementById('d-balance').textContent='S/'+g.balance.toFixed(2);
    document.getElementById('d-tickets').textContent=d.agencias.reduce((a,b)=>a+b.tickets,0);
    let db=document.getElementById('d-balance');
    db.className=g.balance>=0?'g':'r';
  }).catch(()=>{});
}

function crearAgencia(){
  let u=document.getElementById('ag-user').value.trim().toLowerCase();
  let p=document.getElementById('ag-pass').value.trim();
  let n=document.getElementById('ag-nombre').value.trim();
  if(!u||!p||!n){glMsg('Complete todos los campos','err');return;}
  let fd=new FormData(); fd.append('usuario',u); fd.append('password',p); fd.append('nombre',n);
  fetch('/admin/crear-agencia',{method:'POST',body:fd}).then(r=>r.json()).then(d=>{
    let m=document.getElementById('ag-msg');
    if(d.status==='ok'){m.innerHTML=`<div class="msg ok">✅ ${d.mensaje}</div>`;cargarAgencias();}
...
function cargarAgencias(){
  fetch('/admin/lista-agencias').then(r=>r.json()).then(ags=>{
    let html='<div style="overflow-x:auto"><table><thead><tr><th>ID</th><th>Usuario</th><th>Nombre</th><th>Comisión</th><th>Estado</th></tr></thead><tbody>';
    ags.forEach(ag=>{
      html+=`<tr><td>${ag.id}</td><td>${ag.usuario}</td><td>${ag.nombre_agencia}</td>
        <td>${(ag.comision*100).toFixed(1)}%</td>
        <td style="color:${ag.activa?'#4ade80':'#f87171'}">${ag.activa?'✅ ACTIVA':'🚫 INACTIVA'}</td></tr>`;
    });
...
function buscarTicketAdm(){
  let serial=document.getElementById('op-serial').value.trim(); if(!serial)return;
  ...
  .then(r=>r.json()).then(d=>{
    if(d.error){c.innerHTML=`<div style="background:#1a0808;color:#e05050;padding:10px;border-radius:3px;border:1px solid #6b1515">❌ ${d.error}</div>`;return;}
    let col=d.total_ganado>0?'#4ade80':'#4a6090';
    c.innerHTML=`<div style="background:#0d1828;border:2px solid ${col};border-radius:4px;padding:14px">
      <div style="color:var(--teal);font-family:'Oswald',sans-serif;letter-spacing:2px;margin-bottom:8px">TICKET #${serial}</div>
      ...
      ${d.total_ganado>0?`<button onclick="pagarAdm(${d.ticket_id},${d.total_ganado})" style="width:100%;padding:11px;background:#166534;color:#fff;border:2px solid #22c55e;border-radius:3px;font-weight:700;cursor:pointer;font-family:'Oswald',sans-serif;letter-spacing:2px;font-size:.82rem">💰 PAGAR S/${d.total_ganado.toFixed(2)}</button>`:'<div style="color:var(--text2);text-align:center;font-size:.78rem">SIN PREMIO</div>'}
    </div>`;
  });
}
...
function glMsg(msg,tipo){
  ...
  clearTimeout(window._gmt); window._gmt=setTimeout(()=>m.style.display='none',3000);
}

document.addEventListener('DOMContentLoaded',()=>{
  let hoy=new Date().toISOString().split('T')[0];
  document.getElementById('ra-fecha').value=hoy;
  renderLotTabs('lot-tabs-res','zoolo',selLotRes);
  renderLotTabs('lot-tabs-lim','zoolo',selLotLim);
  updateHorarioSelect('zoolo','ra-hora');
  updateHorarioSelect('zoolo','lim-hora');
  renderAnimalesMini('zoolo','animals-mini-res',(k)=>{animalSelAdmin=k;updateSelInfo();});
  renderAnimalesMini('zoolo','animals-lim',(k)=>{animalSelLim=k;updateLimSelInfo();});
  cargarDash(); cargarAgencias(); cargarLimites();
});
</script>
</body></html>'''
