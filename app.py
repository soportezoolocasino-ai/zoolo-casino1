    ...
    try:
        ...
        if trips_t:
            ...
            for t in trips_t:
                ...
                hora_ini = HORARIOS_PERU[0]   # 09:00 AM
                hora_fin = HORARIOS_PERU[-1]  # 06:00 PM
                def fmt_h(h):
                    import re as _re
                    m2 = _re.match(r'(\d+):(\d+) (AM|PM)', h.strip())
                    if m2:
                        hh, mm, ap = m2.group(1), m2.group(2), m2.group(3)
                        return f"{int(hh)}{ap}" if mm=='00' else f"{int(hh)}:{mm}{ap}"
                    return h.replace(' ','')
                lineas.append(f"-------TRPLZOOL----------")
                lineas.append(f"DESDE {fecha_hoy_fmt} Sorteo {fmt_h(hora_ini)} PERU")
                lineas.append(f"HASTA {fecha_hoy_fmt} Sorteo {fmt_h(hora_fin)} PERU")
                lineas.append(f"(10 sorteos fijos del dia: 9AM a 6PM)")
                partes = []
                for n in nums:
        ...
        lineas += ["------------------------",
                   f"*TOTAL: S/{fmt(total)}*",
                   "",
                   "ZOOLO CASINO: Valido 10 sorteos fijos del dia (9AM-6PM)" if trips_t else "",
                   "Buena Suerte! 🍀",
